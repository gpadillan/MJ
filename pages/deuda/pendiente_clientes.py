...
def render():
    ...
    if cols_18_21:
        ...
        st.markdown(
            f"**👥 Total clientes con deuda en 2018–2021:** `{df1['Cliente'].nunique()}` "
            f"– 🏅 Total deuda: `{total_deuda_18_21:,.2f} €`"
        )
    ...
    if cols_22_25:
        ...
        st.markdown(
            ...
            f"– 🏅 Total deuda: `{total_deuda_22_25:,.2f} €`"
        )
    ...
    if '2018_2021' in resultado_exportacion:
        total_deuda_global += resultado_exportacion['2018_2021'].iloc[:, 1:].sum().sum()
    if '2022_2025' in resultado_exportacion:
        total_deuda_global += resultado_exportacion['2022_2025'].iloc[:, 1:].sum().sum()

    st.markdown(
        f"### 🧮 **TOTAL GLOBAL de clientes únicos con deuda:** `{len(total_clientes_unicos)}` "
        f"– 🏅 **Total deuda:** `{total_deuda_global:,.2f} €`"
    )
